...
Gripper: prismatic joint, 0.0m (closed) to 0.032m (open)
         Runtime-adjustable: Kp via /{side}_gripper_stiffness_controller/commands
                             Kd via /{side}_gripper_damping_controller/commands
         Default: Kp=2.0, Kd=0.1 | Safety floor: Kp_min=0.3, Kd_min=0.05
         Position via: /right_gripper_controller/gripper_cmd (GripperActionController)
         VERIFIED (2026-04-17): Kp/Kd adjustable at runtime, G/S modes confirmed on hardware
Cameras:
...
# Spawn gripper stiffness/damping controllers
...
>
> **HARDWARE WORK COMPLETED (2026-04-17):**
> - [x] `v10_simple_hardware.hpp`: kp_/kd_/kp_min_/kd_min_ extended to 8 elements (gripper)
> - [x] `v10_simple_hardware.cpp`: gripper stiffness/damping exposed as command interfaces
> - [x] `v10_simple_hardware.cpp`: write() uses kp_commands_[ARM_DOF] instead of hardcoded GRIPPER_KP
> - [x] `openarm_v10_bimanual_controllers.yaml`: gripper stiffness/damping FCCs added
> - [x] `impedance_gui.py`: G/S toggle buttons + Kp/Kd sliders for gripper, verified on hardware
> - [x] Gripper stroke corrected to 32mm (actual measurement)

**Remaining work (C++ compliance controller integration):**

The hardware-level gripper Kp/Kd control works. Currently the GUI uses ForwardCommandControllers
to set gripper Kp/Kd. The remaining task is to integrate the gripper into the compliance_controller
natively so it can be managed alongside arm joints via a single `/set_impedance` topic.
...
- [x] Gripper closes gently with Kp=1.0 (noticeably softer than Kp=5.0)
- [ ] Gripper Kp/Kd adjustable via /set_impedance topic (compliance controller integration)
- [ ] Gripper force feedback published on ~/grip_force topic
- [ ] Can hold a plastic cup without crushing it (Kp=1.5) -- systematic test
- [ ] Can hold a heavy object without dropping (Kp=4.0) -- systematic test
- [x] Impedance profile manager includes gripper Kp in each profile
...
> This paper validates our architecture: VLA (~3 Hz action chunks) + VLM (~1 Hz impedance coach)
> + VIC (1000 Hz compliant execution). They show VLA-only baselines fail <54% on contact tasks
> with force limits, but adding a VLM-guided impedance adaptor improves safety and success rates.
> Their 3-tier architecture (VLM→impedance params, VLA→position targets, VIC→safe execution)
> maps directly to our compliance_controller (VIC) + impedance_profile_manager + VLA node.
> **Both C1 and C2 must read this paper before starting Phase 4.**
...
#### Task 4.2: Impedance Scheduler (Proprioceptive + VLM-guided)
...
**Interface specification:**
```python
# Two-tier impedance scheduling (inspired by CompliantVLA-adaptor):
#
# Tier 1: Proprioceptive (1000 Hz, safety-critical)
#   Subscribes:
...
#     else:                                phase = "grasp"
#   Force threshold note: J1/J2 have ~0.6 Nm residual.
#
# Tier 2: VLM semantic coach (~1 Hz, context-aware) [FUTURE]
#   Uses VLM (e.g., SmolVLM-2 or API) to interpret visual context
#   and generate context-aware stiffness/damping suggestions.
#   Example: VLM sees "fragile glass" → suggests low Kp
#   NOT safety-critical — Tier 1 always has final say.
#
# Publishes:
...
|-------|------|-------------|----------|--------|--------|
| 1 | 1.1 | Left arm compliance validation | C1 | PASS | Sim validated 2026-04-29 |
...
| 2 | 2.1 | IK executor (MoveIt) | C1 | PASS | TRAC-IK fix by C1 — 10/10 success, 0.16-1.77s/plan |
| 2 | 2.2 | Vision pipeline integration | C2 | PASS | Camera + YOLO + depth validated on HW 2026-05-01 |
| 2 | 2.3 | Demo 1 visual reaching | C2 | PASS | V-10: 15/19 (79%). Relaxed orientation fix. See AGENT_C2_VISION.md |
| 2 | 2.4 | Impedance profile manager | C1 | PASS | Sim validated 2026-04-30 |
| 2 | 2.5 | Gripper impedance control | C1 | PASS | C++ merged 2026-04-30 |
| 2 | Gate | Phase 2 gate review | R | PASS | COND PASS → C2 fixes done: dead code, param cache. Built zero warnings. |

---

## ✅ BLOCKER RESOLVED: IK Solver (2026-05-01)

### IK Solver Fixed — TRAC-IK Installed by C1

**Resolution**: C1 replaced KDL with TRAC-IK. Planning now succeeds (10/10 reachable poses, 0.16-1.77s).
**Retest by C2**: Reach #1 succeeded end-to-end (detect → plan 3.5s → move → hold 2s → contact).
**Remaining**: Multi-reach cycle needs real hardware validation (sim arm state drifts after first reach).

**Root Cause**: The MoveIt config uses `kdl_kinematics_plugin/KDLKinematicsPlugin` as the IK solver.
KDL's Newton-Raphson solver is **unreliable for 7-DOF redundant manipulators** — it often fails
to find valid IK solutions even for reachable targets. This is a well-known limitation.

**What C2 Has Verified Works**:
- ✅ Camera (D435i) streams RGB + depth at 25/3 Hz
- ✅ YOLO detects objects (bottle at 86% confidence)
- ✅ 3D positions are published in world frame with correct TF
- ✅ Visual reach demo state machine runs correctly (IDLE → TARGETING → TRANSIT)
- ✅ Impedance phase switching works
- ✅ Compliance controller activates and runs at 100 Hz
- ❌ MoveIt planning fails 100% of the time for any Cartesian target

**Recommendation — Agent-O should assign a new task for IK solver integration**:

| Option | Effort | Reliability | Notes |
|--------|--------|-------------|-------|
| **TRAC-IK** | Low (apt install) | Good | Drop-in KDL replacement, uses SQP + KDL dual strategy |
| **IKFast** | Medium (generate) | Best | Analytical solver, requires OpenRAVE to generate plugin |
| **BioIK** | Medium (build) | Good | Gradient-based, great for redundant arms |
| **Custom analytical** | High | Best | If OpenArm has spherical wrist, can derive closed-form |

**Fastest fix**: Install `ros-humble-trac-ik-kinematics-plugin`, change `kinematics.yaml`:
```yaml
right_arm:
...
**Location of config**: `~/ros2_ws/src/core/openarm_ros2/openarm_bimanual_moveit_config/config/kinematics.yaml`
